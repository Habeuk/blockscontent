...
/**
 * Provides a form for reverting a Blocks contents revision.
 *
 * @ingroup blockscontent
 */
class BlocksContentsRevisionRevertForm extends ConfirmFormBase {
  ...
  public function getFormId() {
    return 'blocks_contents_revision_revert_confirm';
  }
  ...
  public function submitForm(array &$form, FormStateInterface $form_state) {
    ...
    $this->logger('content')->notice('Blocks contents: reverted %title revision %revision.', ['%title' => $this->revision->label(), '%revision' => $this->revision->getRevisionId()]);
    $this->messenger()->addMessage(t('Blocks contents %title has been reverted to the revision from %revision-date.', ['%title' => $this->revision->label(), '%revision-date' => $this->dateFormatter->format($original_revision_timestamp)]));
   ...
   *
   * @param \Drupal\blockscontent\Entity\BlocksContentsInterface $revision
   ...
   */
  protected function prepareRevertedRevision(BlocksContentsInterface $revision, FormStateInterface $form_state) {
